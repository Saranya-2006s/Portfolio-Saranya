...
// Endpoint: Ask AI about the developer (Portfolio Q&A)
...
2. If asked about something not mentioned, politely explain what is known and offer to connect them via the email (${portfolioContext?.profile?.email}).
...
// Endpoint: AI Enhancer (for profile bio or project description)
app.post('/api/ai/enhance', async (req, res) => {
  try {
    const { type, content, context } = req.body;
    if (!content) {
      return res.status(400).json({ error: 'Content is required to enhance' });
    }

    const ai = getAiClient();
    let prompt = '';
    if (type === 'bio') {
      prompt = `Polish and improve the following professional developer bio to make it engaging, clean, clear, and impactful for tech recruiters and tech leads:\n\n"${content}"\n\nDeveloper Title: ${context?.title || 'Software Engineer'}`;
    ...
    return res.status(500).json({
      error: err.message || 'Failed to enhance text.',
    });
  }
});
